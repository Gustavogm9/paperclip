/**
 * Bootstrap admin script for Paperclip running in containers (Railway, Docker, etc).
 *
 * Runs on every container start. Checks whether any instance_admin exists.
 * - If an admin exists, exits silently with code 0.
 * - If no admin exists, checks whether a non-expired, non-revoked bootstrap_ceo
 *   invite is already active. If so, reuses it (prints URL). Otherwise creates
 *   a new one, prints the invite URL to stdout so it's captured in container logs.
 *
...
async function main() {
  ...
  try {
    ...
    // Do not exit non-zero — server must still start so the UI can show
    // "Instance setup required" and user can retry manually.
  } finally {
